...
运行中的Redis专用Server可以调用`Server::SetRedisMaxConnections()`原子更新上限。调低上限不会关闭已有连接；在当前连接数降到新上限以下之前，新连接会被拒绝。调高上限会在后续连接准入检查中生效，设置为0表示取消限制。以不限制值启动的Redis专用Server也可以通过该接口启用限制。
