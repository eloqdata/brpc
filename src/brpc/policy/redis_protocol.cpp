...
ParseResult ParseRedisMessage(butil::IOBuf* source, Socket* socket,
    ...
    if (server) {
        ...
        if (FLAGS_use_io_uring && !FLAGS_enable_ssl_io_uring) {
            // Registered ring write buffers bypass SSL encryption; with
            // -enable_ssl_io_uring replies stay in IOBufs so that TLS
            // sockets can encrypt them in Socket::DoWrite.
